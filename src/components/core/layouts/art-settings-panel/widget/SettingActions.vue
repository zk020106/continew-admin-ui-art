...
const handleResetConfig = async () => {
  try {
    ...
    // 主题相关 - ✅ 关键修复：直接调用 useTheme，不修改 settingStore
    ...
    // 界面显示
    settingStore.showMenuButton = config.showMenuButton
    settingStore.showFastEnter = config.showFastEnter
    settingStore.showRefreshButton = config.showRefreshButton
    settingStore.showCrumbs = config.showCrumbs
    settingStore.setWorkTab(config.showWorkTab)
    settingStore.showLanguage = config.showLanguage
    settingStore.showNprogress = config.showNprogress
    if (config.showSettingGuide) {
    ...
    // 功能设置
    settingStore.autoClose = config.autoClose
    settingStore.uniqueOpened = config.uniqueOpened
    settingStore.colorWeak = config.colorWeak
    settingStore.refresh = config.refresh
    settingStore.setholidayFireworksLoaded(config.holidayFireworksLoaded)

    // 样式设置
    settingStore.boxBorderMode = config.boxBorderMode
